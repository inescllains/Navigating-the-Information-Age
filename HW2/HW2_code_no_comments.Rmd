...
Create three separate dfs for each category 
  ...
  -> You can use 1 for every category because it doesnt really matter which one it is, it only matters that it is a different category. 
```{r}
...
-> d$label_true - this is a vector that contains the true labels for each item.
-> d$label_assigned - this is a vector containing the labels assigned by the model/classifier.
-> precision - this calculates the precision based on these two vectors. 
  -> It automatically knows that that precision is "true positive/(true positive+false negative)"        because it comes from the metrics library that already knows this.
```{r}
